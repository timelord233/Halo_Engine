...
namespace Halo {
	...
	Buffer& Material::GetUniformBufferTarget(ShaderUniformDeclaration* uniformDeclaration)
	{
		...
		HL_CORE_ASSERT(false, "Invalid uniform declaration domain! Material does not support this shader type.");
	...
	MaterialInstance::MaterialInstance(const SharedPtr<Material>& material)
		: m_Material(material)
	{
