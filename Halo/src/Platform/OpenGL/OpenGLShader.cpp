...
#if UNIFORM_LOGGING
#define HL_LOG_UNIFORM(...) HL_CORE_WARN(__VA_ARGS__)
#else
#define HZ_LOG_UNIFORM
	...
	SharedPtr<OpenGLShader> OpenGLShader::CreateFromString(const std::string& source)
	{
		SharedPtr<OpenGLShader> shader = std::make_shared<OpenGLShader>();
		shader->Load(source);
		return shader;
	}
	...
	std::string OpenGLShader::ReadShaderFromFile(const std::string& filepath) const
	{
		...
		{
			HL_CORE_ASSERT(false, "Could not load shader!");
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			HL_CORE_ASSERT(type == "vertex" || type == "fragment" || type == "pixel" || type == "compute", "Invalid shader type specified");
	...
	void OpenGLShader::ResolveAndSetUniforms(const UniquePtr<OpenGLShaderUniformBufferDeclaration>& decl, Buffer buffer)
	{
		const ShaderUniformList& uniforms = decl->GetUniformDeclarations();
	...
	void OpenGLShader::ResolveAndSetUniform(OpenGLShaderUniformDeclaration* uniform, Buffer buffer)
	{
		...
		HL_CORE_ASSERT(uniform->GetLocation() != -1, "Uniform has invalid location!");
		...
		case OpenGLShaderUniformDeclaration::Type::STRUCT:
			...
			break;
		default:
			HL_CORE_ASSERT(false, "Unknown uniform type!");
		}
	}
	...
	void OpenGLShader::UploadUniformFloat(const std::string& name, float value)
	{
		...
		if (location != -1)
			...
			HL_LOG_UNIFORM("Uniform '{0}' not found!", name);
	}
